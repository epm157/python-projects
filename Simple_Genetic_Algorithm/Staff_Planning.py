...
def employee_present(employee, time):
    ...
    return ((time >= employee_start_time) and (time < employee_end_time))
    ...
    for day in staff_planning:
        ...
        for employee in day:
            ...
            for hour in range(0, 24):
                employee_present_hour.append(employee_present(employee, hour))
            hourly_staff_day.append(employee_present_hour)
        hourly_staff_week.append(hourly_staff_day)

    hourly_staff_week = np.array(hourly_staff_week).sum(axis=1)
    return hourly_staff_week
...
def create_parent_generation(n_parents, n_days = 7, n_staff = 11):
    ...
    for i in range(n_parents):
        parent = generate_random_staff_planning(n_days, n_staff)
...
best_planning = gen_algo(hourlystaff_needed, n_iterations = 2000)
print(best_planning)
...
print(random_staff_planning)

# show the cost of this random week planning
cost1 = cost(staffplanning_to_hourlyplanning(random_staff_planning), hourlystaff_needed)
print(cost1)
'''
