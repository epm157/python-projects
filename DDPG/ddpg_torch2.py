...
class CriticNetwork(nn.Module):
    ...
    def __init__(self, beta, input_dims, fc1_dims, fc2_dims, n_actions, name, chkpt_dir='tmp/ddpg'):
        ...
        #self.fc1.weight.uniform_(-f1, f1)
        ...
        #self.fc2.weight.uniform_(-f2, f2)
        ...
        #self.q.weight.uniform_(-f3, f3)
        #self.q.bias.uniform_(-f3, f3)
...
class Agent:
    def __init__(self, alpha, beta, input_dims, tau, gamma=0.99, n_actions=2, max_size=1_000_000, layer1_size=400, layer2_size=300, batch_size=64):
        self.gamma = gamma
        self.tau = tau
    ...
    def learn(self):
        ...
        new_critic_value = self.target_critic.forward(new_state, target_actions)
        critic_value = self.critic.forward(state, action)

        target = []
        for j in range(self.batch_size):
            target.append(reward[j] + self.gamma*new_critic_value[j]*done[j])
...
for i in range(1000):
    ...
    if i > 0 and i % 100 == 0:
        filename = 'LunarLander-2-' + str(i) + '.png'
