...
class Agent(object):
    ...
    def build_policy_network(self):
        inputt = tf.keras.layers.Input(shape=(self.input_dims,))
        advantages = tf.keras.layers.Input(shape=[1])
        dense1 = tf.keras.layers.Dense(self.fc1_dims, activation='relu')(inputt)
        ...
        policy = tf.keras.models.Model(inputs=[inputt, advantages], outputs=[probs])

        policy.compile(optimizer=tf.keras.optimizers.Adam(lr=self.lr), loss=custom_loss)

        predict = tf.keras.models.Model(inputs=[inputt], outputs=[probs])
