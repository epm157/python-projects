...
class CNNNetwork(nn.Module):
    def __init__(self, lr, batch_size, n_classes, epochs):
        ...
        self.network = nn.Sequential(self.cell1, self.cell2, self.cell3, self.max_pool1, self.cell4, self.cell5, self.cell6, self.max_pool2)
        self.fc = nn.Linear(in_features=256, out_features=self.n_classes)
    ...
    def forward(self, input):
        output = T.tensor(input).to(self.device)
    ...
    def _train(self):
        ...
        for i in range(self.epochs):
            ...
            for j , (input, label) in enumerate(self.train_data_loader):
                ...
                wrong = T.where(classes != label, T.tensor([1.0]).to(self.device), T.tensor([0.0]).to(self.device))
                ...
                loss.backward()
                self.optimizer.step()
            print('Finish epoch ', i, 'total loss %.3f training accuracy %.3f' % (ep_loss, np.mean(ep_acc)))
